...
_SPACE_HANDLING_RULE = (
    "字段值的空格与换行处理（务必执行）：OCR 常在字母 / 数字 / 中文之间误插空格，"
    "或因排版把同一字段（尤其是地址）折成多行。抽取时一律去掉这些【误插的无意义空格】和【换行符】，"
    "把折行字段合并成一行；中文地址写成连续字符串、不得保留任何空格"
    "（如 \"4号 紫禁城敬事房\" → \"4号紫禁城敬事房\"）；"
    "只保留原文【本就有】的有意义空格（如英文单词间隔）。"
)
...
"""
    + "3. " + _SPACE_HANDLING_RULE + "\n"
    + """4. 字段名与取值口径遵循上文【统一口径】（严格照抄 OCR 原文写法、值里不写字段名、不编造）。
