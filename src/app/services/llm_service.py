...
5. 【核心】提取每个值时，必须先判断它「是不是有效数据」，再决定写什么：
   - 区分两类内容：
...
应提取为：
{
  "tables": [
    {
      "table_index": 0,
      "headers": ["姓名", "性别", "民族", "出生", "住址", "公民身份号码"],
      "rows": [
        {"姓名": "蒋飞成", "性别": "男", "民族": "汉", "出生": "1976年3月20日", "住址": "湖南省邵阳县五峰铺镇白 田村下尤铺组17号", "公民身份号码": "430523197603204314"}
      ]
    }
  ]
}
...
该示例体现的要点（务必遵守）：
- 标签与值分行时，把上一行/上一字段名对应的下一行内容作为该字段的值（如 "姓名" 对应 "蒋飞成"）。
- 标签与值连写无分隔符时也要正确拆分："性别男" → 字段"性别"=值"男"；"民族汉" → "民族"="汉"；"出生1976年3月20日" → "出生"="1976年3月20日"。值里【不得】残留字段名。
- 标签与值之间夹着的图片占位（如（图片占位）、image、<!-- image -->）一律忽略，直接取其后的真实数据作为值（如 "公民身份号码" 对应 "430523197603204314"）。
...
5. 【核心】提取每个值时，必须先判断它「是不是有效数据」，再决定写什么：
   - 区分两类内容：
     · 字段标签（不是数据）：字段名、表头、行标题、带冒号的行首，例如 "合同编号"、"甲方（盖章）："、"签订日期"。它们是占位/说明，不是可填写的事实。
     · 字段值（是有效数据）：标签对应的一条具体业务事实，例如 "HT-2024-001"、"某某公司"、"2024.1.5"。
   - 单元格【只能填"值"】，绝不可把标签本身、表头文字、或 "甲方：某某公司" 这种带标签前缀的整串当作值。
   - 若某份文档某字段【只有标签、没有对应数据】，该单元格【留空 ""】，不要把标签再写回去。
   - 不要把纯说明/装饰性文字当作值，例如 "注：……"、"详见第X页"、"以下空白"、"（盖章处）"。
   - 判断标准：问自己"这串文字是不是一个具体的、可填写的事实？"——是才写进值；若它只是字段名或说明，则不是有效数据，留空。
   - 字段值只承载「该字段自身属性」：绝不要把【不属于该字段属性】的内容塞进字段。具体：
     · 不要把说明、备注、与表格无关的文字写进字段值；
     · 不要把多个字段的内容拼接到同一个字段里（若原文把多项写在同处，应拆分为各自对应的字段，而非全部塞进某一个）；
...
6. 【核心·按值的类型与业务含义判断字段归属】不要只会套用示例里的固定字段；要能根据「值的类型与业务含义」自主推断它属于哪个字段：
   ...
   - 当某段文字没有明确标签、或标签与示例不同（如合同里写「供方」而非「甲方」、写「买受人」而非「乙方」）时，依据【值的类型与上下文】把它归入语义最匹配的业务字段，并为该字段起一个合理的中文业务名。
   - 要点：字段识别靠「这个值是什么」，不是「示例里有没有」。任何新文档都要用这套通用方法推断字段，不要只会认示例中的那几种。
7. 不得编造数据。若某份文档缺少某个字段，该单元格留空（""）。
8. 只返回合法的 JSON，不要附带任何解释说明，格式如下：
{
  "tables": [
    {
      "title": "批次汇总",
...
示例（单份文档 → 一行，输入 → 正确输出）：
DOCUMENT 1 的 OCR 文本（标签与值分行、有的连写无分隔符、标签与值之间夹图片占位）：
  ...
  "tables": [
    {
      "title": "批次汇总",
      "headers": ["姓名", "性别", "民族", "出生", "住址", "公民身份号码"],
      "rows": [ {"姓名": "蒋飞成", "性别": "男", "民族": "汉", "出生": "1976年3月20日", "住址": "湖南省邵阳县五峰铺镇白 田村下尤铺组17号", "公民身份号码": "430523197603204314"} ]
    }
  ]
}

要点：连写无分隔符也要拆分（"性别男"→"性别"="男"）；标签与值之间的图片占位（（图片占位）/image/<!-- image -->）忽略，取其后真实数据；值不含字段名、不塞入无关内容。多份文档就按此规则各生成一行，组合进同一张表。
（此示例仅演示一种文档；其它类型文档请用上文『按值判断字段归属』通用方法自主推断字段，不要只会认示例中的字段。）"""
